// ==UserScript==
// @name         ChatGPT Sidebar Organizer
// @namespace    local.lex.chatgpt-organizer
// @version      2.2.0
// @description  Organize ChatGPT conversations with local sections, status tags, search, filters, and custom ordering.
// @author       Lex + Codex
...
(() => {
  ...
  const STATUS = {
    none: { label: 'No status', color: '#8b8b8b' },
  ...
  function normalizeState(parsed) {
      ...
      Object.values(rawChats).slice(0, MAX_CHATS).forEach(rawChat => {
        ...
        const chat = {
          key,
          title: cleanedTitle && !isPlaceholderTitle(cleanedTitle) ? cleanedTitle.slice(0, 240) : 'Fetching title…',
          url,
  ...
  function saveState() {
    try {
      ...
      console.warn('[ChatGPT Sidebar Organizer] Could not save local data.', error);
      if (!storageWarningShown) {
        storageWarningShown = true;
        setTimeout(() => toast('Could not save organizer data. Browser storage may be full or unavailable.'), 0);
      }
      return false;
    }
  }
  ...
  function nextSectionName() {
    ...
    if (!existing.has('new section')) return 'New section';
    let number = 2;
    while (existing.has(`new section ${number}`)) number += 1;
  ...
  function createSection(name, color = '#10a37f') {
    ...
    if (!cleanName) return { section: null, error: 'Enter a section name' };
    const duplicate = state.sections.find(section =>
      section.name.trim().toLocaleLowerCase() === cleanName.toLocaleLowerCase()
    );
    if (duplicate) return { section: duplicate, error: 'A section with this name already exists' };
  ...
  function colorPaletteHtml() {
    return `<div class="cso-color-palette" aria-label="Preset section colors">
      ${SECTION_COLORS.map(color => `<button type="button" data-preset-color="${color}" style="--swatch:${color}" title="${color}" aria-label="Choose color ${color}"></button>`).join('')}
    </div>`;
  }
  ...
  function schedulePendingOpenVerification(delay = 1200) {
    ...
    pendingOpenTimer = setTimeout(() => {
      ...
      if (missing || redirectedAway) {
        removeChatFromOrganizer(pending.key, 'This conversation no longer exists and was removed from Organizer.');
        return;
      }
      if (elapsed >= 8000) {
        clearPendingOpen();
  ...
  function installNativeDeleteSync() {
    ...
    document.addEventListener('click', event => {
      ...
      pendingNativeDelete = null;
      setTimeout(() => removeChatFromOrganizer(key, 'The deleted conversation was removed from Organizer.'), 800);
    }, true);
  }
  ...
  function upsertChat(rawUrl, title, options = {}) {
    ...
    const fallbackTitle = existing.title && !isPlaceholderTitle(existing.title) ? existing.title : 'Fetching title…';
    const now = Date.now();
  ...
  async function responseError(response) {
    ...
    if (response.status === 401 || response.status === 403) return 'Authentication or account permission failed. Refresh ChatGPT and try again.';
  ...
  async function mutateConversation(key, kind, title = '') {
    ...
    if (!id) throw new Error('Direct actions currently support standard ChatGPT conversations only, not Work/Codex items.');
  ...
  async function renameChatOnChatGPT(key, title, button) {
    const originalLabel = button?.textContent || 'Rename';
    if (button) { button.disabled = true; button.textContent = 'Renaming…'; }
    try {
      ...
      closeModal();
      queueRender();
      toast('ChatGPT conversation renamed.');
    } catch (error) {
  ...
  function openRenameChat(key) {
    const chat = state.chats[key];
    if (!chat) return;
    const layer = openModal('Rename ChatGPT conversation',
      `<label class="cso-field">New title<input name="native-title" value="${esc(chat.title)}" maxlength="128"></label>
       <div class="cso-note">This changes the title stored by ChatGPT. The conversation does not need to be visible in the native sidebar.</div>`,
      '<button data-rename-native class="cso-primary">Rename</button><button data-modal-close>Cancel</button>');
  ...
  function openDeleteChat(key) {
    const chat = state.chats[key];
    if (!chat) return;
    const layer = openModal('Delete ChatGPT conversation',
      `<div>Delete “${esc(chat.title)}”?</div>
       <div class="cso-note">This deletes the conversation from your ChatGPT account, not just from Organizer. This action cannot be undone.</div>`,
      '<button data-delete-native class="cso-danger">Delete conversation</button><button data-modal-close>Cancel</button>');
    ...
    button.addEventListener('click', async () => {
      ...
      try {
        ...
        closeModal();
        toast('Conversation deleted and removed from Organizer.');
      } catch (error) {
  ...
  async function scanAllHistory() {
    ...
    const originalLabel = button?.textContent || 'Reindex';
    if (button) {
      button.disabled = true;
      button.textContent = 'Finding history…';
    }
    ...
    try {
      ...
      if (!scroller) {
        toast('No scrollable native history list was found. Visible conversations were still indexed.');
        return;
      }
      ...
      for (let step = 0; step < 180; step += 1) {
        if (!scroller.isConnected) throw new Error('The native history list changed while it was being scanned.');
        const result = harvestNativeChats({ includeCurrent: false, persist: false });
        changed = result.changed || changed;
        currentCount += result.added;
        if (button) button.textContent = `Indexing… ${currentCount}`;
      ...
      toast(added > 0 ? `Index complete: ${added} new conversation${added === 1 ? '' : 's'}.` : 'Index complete: no new conversations found.');
    } catch (error) {
      console.warn('[ChatGPT Sidebar Organizer] Reindexing stopped early.', error);
      toast(error?.message || 'Reindexing stopped because the page changed unexpectedly.');
    } finally {
  ...
  function ensureCurrentChat() {
    ...
    if (pending && key !== pending.previousKey && !chat.inProject &&
      ...
      setTimeout(() => toast(`New conversation added to “${sectionName}”.`), 100);
    }
    saveState();
    return key;
  }
  ...
  function startNewChatInSection(sectionId) {
    ...
    if (!section) {
      toast('This section no longer exists.');
      return;
    }
  ...
  function sectionOptions(selected = '') {
    return [
      `<option value="" ${selected === '' ? 'selected' : ''}>Unsectioned</option>`,
      ...[...state.sections]
  ...
  function chatRow(chat) {
    ...
    const statusPill = chat.status !== 'none'
      ...
      : '';
    const typePill = chat.type === 'work' ? '<span class="cso-type-tag">Work</span>' : '';
    return `
      <div class="cso-chat" data-key="${esc(chat.key)}" title="${esc(chat.title)}">
        <span class="cso-drag-handle" draggable="true" aria-label="Drag to reorder" title="Drag to reorder">⠿</span>
        <a href="${esc(chat.url)}" data-action="open-chat"><span class="cso-chat-title">${esc(chat.title)}</span>${typePill}${statusPill}</a>
        <button class="cso-row-action cso-rename" data-action="rename-chat" aria-label="Rename ChatGPT conversation" title="Rename">✎</button>
        <button class="cso-row-action cso-delete" data-action="delete-chat" aria-label="Delete ChatGPT conversation" title="Delete">×</button>
        <button class="cso-more" data-action="edit-chat" aria-label="Edit section and status" title="Edit section and status">•••</button>
      </div>`;
  }
  ...
  function groupedChatHtml() {
    ...
    if (!ordered.length && !Object.keys(state.chats).length) {
      return `<div class="cso-empty">
        <strong>No sections or indexed conversations yet</strong><br>
        <button data-action="manage-sections" class="cso-inline-action">＋ Create first section</button>
        <button data-action="scan" class="cso-inline-action">Index native history</button>
      </div>`;
    }
      ...
      parts.push(`
        <section class="cso-section cso-colored-section" data-section="${esc(section.id)}" style="--section:${esc(section.color)}">
          <div class="cso-section-bar">
            ...
            <button class="cso-new-chat" data-action="new-chat-in-section" data-section-id="${esc(section.id)}" title="Create a conversation in this section" aria-label="Create a conversation in ${esc(section.name)}">＋</button>
          </div>
          <div class="cso-section-items" ${section.collapsed ? 'hidden' : ''}>${items.map(chatRow).join('') || '<div class="cso-empty-mini">No conversations</div>'}</div>
        </section>`);
    }
    ...
    if (unsectioned.length || !chats.length) {
      parts.push(`
        <section class="cso-section cso-unsectioned" data-section="">
          <div class="cso-section-head cso-static"><span class="cso-chevron">•</span><span class="cso-section-name">Unsectioned</span><span class="cso-count">${unsectioned.length}</span></div>
          <div class="cso-section-items">${unsectioned.map(chatRow).join('') || '<div class="cso-empty">No conversations match the current filters</div>'}</div>
        </section>`);
    }
    return parts.join('');
  }
  ...
  function reorderChat(dragKey, targetKey, placeAfter) {
    ...
    if (sectionId !== (target.sectionId || '')) {
      toast('Conversations can only be reordered within the same section.');
      return false;
    }
  ...
  function renderSectionToc(root) {
    ...
    if (hasUnsectioned) {
      rows.push('<button data-action="jump-section" data-section-id="" title="Unsectioned"><span class="cso-toc-dot cso-toc-neutral"></span><span>Unsectioned</span></button>');
    }
    toc.innerHTML = `<div class="cso-toc-title">SECTIONS</div>${rows.join('') || '<div class="cso-toc-empty">No sections</div>'}`;
  ...
  function jumpToSection(root, sectionId) {
    ...
    requestAnimationFrame(() => {
      ...
      if (!target) {
        toast('This section is not visible under the current filters.');
        return;
      }
  ...
  function buildRoot() {
    ...
    root.id = APP_ID;
    root.innerHTML = `
      <button class="cso-launch" data-action="toggle-app" aria-label="Open Chat Organizer">☰ <span>Organizer</span></button>
      <aside class="cso-panel" aria-label="ChatGPT Sidebar Organizer">
        <div class="cso-header">
          <div class="cso-window-drag" data-window-drag title="Drag window"><span class="cso-window-grip">⠿</span><strong>Chat Organizer</strong><span class="cso-total"></span></div>
          <div class="cso-header-actions">
            <button data-action="batch-edit" title="Batch-organize unsectioned conversations">Batch</button>
            <button data-action="expand-all" title="Expand all sections" aria-label="Expand all sections">▾</button>
            <button data-action="collapse-all" title="Collapse all sections" aria-label="Collapse all sections">▴</button>
            <button data-action="back-to-top" title="Back to top">↑</button>
            <button data-action="manage-sections" title="Manage sections">Sections</button>
            <button data-action="settings" title="Settings and backup">⚙</button>
            <button data-action="toggle-app" title="Close">×</button>
          </div>
        </div>
        <div class="cso-workspace">
          <nav class="cso-section-toc" aria-label="Section directory"></nav>
          <div class="cso-main-column">
            <div class="cso-current"></div>
            <div class="cso-quickbar">
              <button data-quick="all">All</button>
              <button data-quick="unsectioned">Unsectioned</button>
            </div>
            <div class="cso-searchline">
              <input class="cso-search" type="search" placeholder="Search indexed conversations…" autocomplete="off">
              <button data-action="toggle-filters" class="cso-filter-btn">Filter<span class="cso-filter-count"></span></button>
            </div>
            <div class="cso-filters" hidden></div>
            <div class="cso-list"></div>
            <div class="cso-footer"><span>Data stays in this browser</span><button data-action="scan">Reindex</button></div>
          </div>
        </div>
      </aside>
  ...
  function renderCurrent(root) {
    ...
    if (!key && pending && getSection(pending.sectionId)) {
      box.innerHTML = `<span>Your first message will add the new conversation to “${esc(getSection(pending.sectionId).name)}”.</span><button class="cso-cancel-pending" data-action="cancel-pending-new-chat">Cancel</button>`;
      return;
    }
    if (!key || !state.chats[key]) {
      box.innerHTML = '<span>Open a saved conversation to organize it.</span>';
      return;
    }
    const chat = state.chats[key];
    if (chat.inProject) {
      box.innerHTML = '<span>This conversation is in a Project and is hidden from Organizer.</span>';
      return;
    }
    const section = getSection(chat.sectionId);
    const status = STATUS[chat.status] || STATUS.none;
    box.innerHTML = `
      <div class="cso-current-label">CURRENT CONVERSATION</div>
      <button data-action="edit-current" title="Organize current conversation">
        <span>${esc(chat.title)}</span>
        <small>${section ? esc(section.name) : 'Unsectioned'} · ${esc(status.label)}${chat.type === 'work' ? ' · Work' : ''}</small>
      </button>`;
  }
  ...
  function renderFilters(root) {
    ...
    if (!filters.dataset.ready || filters.dataset.sectionSignature !== sectionSignature) {
      ...
      filters.innerHTML = `
      <label>Section<select data-filter="section"><option value="all">All sections</option><option value="unsectioned">Unsectioned</option>${[...state.sections].sort((a,b)=>a.order-b.order).map(s => `<option value="${esc(s.id)}">${esc(s.name)}</option>`).join('')}</select></label>
      <label>Status<select data-filter="status"><option value="all">All statuses</option><option value="none">No status</option><option value="active">Active</option><option value="reference">Reference</option><option value="backlog">Backlog</option></select></label>
      <label>Type<select data-filter="type"><option value="all">All</option><option value="chat">Chat</option><option value="work">Work</option><option value="gpt">GPT</option></select></label>
      <label>Sort<select data-filter="sort"><option value="custom">Custom drag order</option><option value="recent">Recently visited</option><option value="oldest">Least recently visited</option><option value="title">Title A–Z</option></select></label>
      <button data-action="clear-filters" class="cso-clear">Clear filters</button>`;
    ...
    root.addEventListener('click', event => {
      ...
      const actions = {
        ...
        'edit-current': () => {
          ...
          else toast('Open a saved ChatGPT conversation first.');
        },
        'toggle-section': () => {
  ...
  function openChatEditor(key) {
    const chat = state.chats[key];
    if (!chat) return;
    const layer = openModal('Organize conversation', `
      <div class="cso-chat-editor-title"><span>Conversation title</span><strong>${esc(chat.title)}</strong></div>
      <div class="cso-field">
        <span>Section</span>
        <div class="cso-section-picker">
          <select name="section">${sectionOptions(chat.sectionId)}</select>
          <button type="button" data-show-inline-section>＋ New</button>
        </div>
      </div>
      <div class="cso-inline-section-create" hidden>
        <input name="new-section-name" placeholder="Section name" maxlength="40">
        <input type="color" name="new-section-color" value="#10a37f" title="Choose any color">
        <button type="button" data-create-inline-section>Create and select</button>
        ${colorPaletteHtml()}
      </div>
      <label class="cso-field">Status (independent tag)<select name="status">
        ${Object.entries(STATUS).map(([id, item]) => `<option value="${id}" ${chat.status === id ? 'selected' : ''}>${esc(item.label)}</option>`).join('')}
      </select></label>
      <div class="cso-note">The displayed name always matches the real ChatGPT title. Use ✎ on the conversation row to rename it.</div>`,
      '<button data-save class="cso-primary">Save</button><button data-remove>Remove from local index</button>');
    ...
    layer.querySelector('[data-create-inline-section]').addEventListener('click', () => {
      ...
      if (result.error) toast(`${result.error}; the existing section was selected.`);
    ...
    layer.querySelector('[data-remove]').addEventListener('click', () => {
      if (!confirm('Remove this item only from the local Organizer index? This will not delete the ChatGPT conversation.')) return;
  ...
  function openBatchEditor() {
    ...
    if (!chats.length) {
      toast('There are no unsectioned conversations.');
      return;
    }
    const rows = chats.map(chat => {
      const section = getSection(chat.sectionId);
      const status = STATUS[chat.status] || STATUS.none;
      const meta = `${section ? section.name : 'Unsectioned'} · ${status.label}${chat.type === 'work' ? ' · Work' : ''}`;
    ...
    const layer = openModal('Batch-organize unsectioned conversations', `
      <div class="cso-batch-toolbar">
        <input type="search" name="batch-search" placeholder="Search conversations…" autocomplete="off">
      ...
      <div class="cso-batch-controls">
        <label class="cso-field">Section<select name="batch-section"><option value="__keep">Keep unchanged</option>${sectionOptions('__keep')}</select></label>
        <label class="cso-field">Status<select name="batch-status"><option value="__keep">Keep unchanged</option>${Object.entries(STATUS).map(([id, item]) => `<option value="${id}">${esc(item.label)}</option>`).join('')}</select></label>
      </div>
      <div class="cso-note"><span data-batch-count>0 conversations selected</span>. Only fields you choose will be changed.</div>`,
      '<button data-apply-batch class="cso-primary">Apply</button><button data-modal-close>Cancel</button>');
    const list = layer.querySelector('.cso-batch-list');
    const updateCount = () => {
      const count = list.querySelectorAll('input:checked').length;
      layer.querySelector('[data-batch-count]').textContent = `${count} conversation${count === 1 ? '' : 's'} selected`;
    };
    ...
    layer.querySelector('[data-apply-batch]').addEventListener('click', () => {
      ...
      if (!keys.length) {
        toast('Select at least one conversation.');
        return;
      }
      ...
      if (sectionValue === '__keep' && statusValue === '__keep') {
        toast('Choose a section or status to change.');
        return;
      }
      keys.forEach(key => {
        const chat = state.chats[key];
      ...
      saveState(); closeModal(); queueRender();
      toast(`Updated ${keys.length} conversation${keys.length === 1 ? '' : 's'}.`);
    });
  }
  ...
  function sectionManagerRows() {
    return [...state.sections].sort((a, b) => a.order - b.order).map(section => `
      <div class="cso-section-edit" data-id="${esc(section.id)}">
        <span class="cso-section-drag" draggable="true" title="Drag to reorder" aria-label="Drag section to reorder">⠿</span>
        <input type="color" name="color" value="${esc(section.color)}" title="Color">
        <input name="name" value="${esc(section.name)}" maxlength="40">
        <button data-delete-section title="Delete">×</button>
      </div>`).join('');
  }

  function openSectionManager() {
    const layer = openModal('Manage sections', `
      <div class="cso-section-editor">${sectionManagerRows() || '<div class="cso-empty-mini">No sections created</div>'}</div>
      <button data-add-section class="cso-add">＋ New section</button>
      <div class="cso-manager-section-create" hidden>
        <input name="manager-section-name" placeholder="Section name" maxlength="40">
        <input type="color" name="manager-section-color" value="#10a37f" title="Choose any color">
        <button type="button" data-confirm-add-section>Create</button>
        ${colorPaletteHtml()}
      </div>
      <button data-delete-all-sections class="cso-delete-all" ${state.sections.length ? '' : 'disabled'}>Delete all sections</button>
      <div class="cso-note">Deleting a section does not delete its conversations. They become unsectioned.</div>`,
      '<button data-save-sections class="cso-primary">Done</button>');
    layer.addEventListener('click', event => {
      ...
      if (presetColor) {
        ...
        if (!confirm('Delete this section? Its conversations will become unsectioned.')) return;
        ...
        if (!confirm('Delete all sections? All conversations will become unsectioned, but no ChatGPT conversations will be deleted.')) return;
        state.sections = [];
  ...
  function syncSectionInputs(layer) {
    layer.querySelectorAll('.cso-section-edit').forEach(row => {
      ...
      if (!section) return;
      section.name = row.querySelector('[name="name"]').value.trim() || 'Untitled';
      section.color = row.querySelector('[name="color"]').value;
    });
  }

  function openSettings() {
    const layer = openModal('Settings and backup', `
      <div class="cso-settings-grid">
        <button data-export>Export JSON backup</button>
        <label class="cso-file-button">Import JSON<input type="file" accept="application/json,.json" data-import hidden></label>
        <button data-reset class="cso-danger">Clear Organizer data</button>
      </div>
      <div class="cso-note">Backups contain titles, URLs, sections, and statuses, but not conversation content. Export a backup before clearing browser site data.</div>`);
    ...
    layer.querySelector('[data-reset]').addEventListener('click', () => {
      if (!confirm('Clear all local sections, statuses, and indexed records? ChatGPT conversations will not be affected.')) return;
  ...
  async function importData(file) {
    if (!file) return;
    try {
      if (file.size > MAX_IMPORT_BYTES) throw new Error('The backup exceeds the 5 MB import limit.');
      ...
      if (parsed.sections.length > MAX_SECTIONS || Object.keys(parsed.chats).length > MAX_CHATS) {
        throw new Error(`Backup limits: ${MAX_SECTIONS} sections and ${MAX_CHATS.toLocaleString()} conversations.`);
      }
      if (!confirm('Importing will replace the current Organizer data. Continue?')) return;
      state = normalizeState(parsed);
      saveState(); closeModal(); queueRender(); toast('Import complete.');
    } catch (error) {
